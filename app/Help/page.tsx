...
const HelpPage = () => {
  ...
  const faqs = [
    ...
    {
      section: 'My concern is not listed here',
      questions: [
        {
          question: 'Feel free to mail us at:',
          answer: 'gyrateone@gmail.com',
  ...
  useEffect(() => {
    if (section) {
      const sectionIndex = faqs.findIndex(
        (faqSection) => faqSection.section === section
      );
      if (sectionIndex !== -1) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      {faqs.map((section, sectionIndex) => (
        <div key={sectionIndex} id={`section-${sectionIndex}`} className="mb-10">
          <h2 className="text-2xl font-bold mb-6 text-black bg-gradient-to-r from-blue-500 via-green-500 to-orange-600 text-transparent bg-clip-text drop-shadow-lg">
            {section.section}
          </h2>
